...
```{r logo, eval=FALSE, include=FALSE}
library(hexSticker)
library(dplyr)

k <- ggokabeito::palette_okabe_ito(1:8)
r <- col2rgb(k)
d <- bind_cols(
  col = k,
  rgb2hsv(r) |> 
...
# colorspace::hclplot(k)


#
# pie(d$n, col = d$col)
#
# alluvial(d[,1], freq=d$n)
#
...
Alluvial diagram is a variant of a Parallel Coordinates Plot (PCP) but for categorical variables. Variables are assigned to vertical axes that are parallel. Values are represented with strata (category blocks) on each axis. Observations are represented with *alluvia* (sing. "alluvium") spanning across all the axes.

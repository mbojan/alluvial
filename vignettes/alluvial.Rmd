...
tit %>%
  mutate(
    ...
    k = RColorBrewer::brewer.pal(4, "Paired")[ match(ss, sort(unique(ss))) ]
  ) -> tit
...
alluvial(tit[,1:4], freq=tit$Freq,
         hide = tit$Freq < 10,
         col = tit$k,
         border = adjustcolor(tit$k, 0.2),
         blocks=FALSE,
         ordering = list(
           NULL,
           NULL,
           order(tit$Age, tit$Survived == "No"),
           order(tit$Survived, tit$Sex)
         )
)
```
...
Colors of the alluvia can be customized with `col`, `border` and `alpha` arguments.
...
alluvial(tit3d[,1:3], freq=tit3d$n, alpha=1, xw=0.2,
